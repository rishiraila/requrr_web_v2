...
export async function GET(req, { params }) {
  ...
  const [rows] = await db.query('SELECT * FROM services WHERE id = ? AND user_id = ?', [id, user.id]);
...
export async function PUT(req, { params }) {
  ...
  await db.query(
    ...
    [name, description, billing_type, billing_interval || null, base_price, is_active ?? 1, id, user.id]
  );
